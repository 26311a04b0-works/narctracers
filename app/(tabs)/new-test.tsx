...
import { SUBSTANCE_OPTIONS, GUIDED_STEPS } from '@/constants/mockData';
...
export default function NewTestScreen() {
  ...
  const handleSave = () => {
    if (!sampleId.trim() || !result || !capturedImageUri) {
      Alert.alert(
        'Missing Fields',
        'Please fill in Sample ID, capture an evidence photo, and select a result before saving.'
      );
      return;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        {step === 'form' && (
          <View style={styles.formSection}>
            ...
            <View style={{ marginTop: 16 }}>
              <Dropdown
                label="Suspected Substance (Optional)"
                value={substance}
                options={SUBSTANCE_OPTIONS}
                onSelect={setSubstance}
              />
            </View>
            <Text style={styles.optionalHint}>
              Substance may be unknown at the time of the field test. You can select it later.
            </Text>
            ...
            <View style={styles.notesRow}>
              ...
              <TextInput
                ...
                placeholder="Type observation notes or use the voice note button..."
                placeholderTextColor={Colors.darkGray}
                value={notes}
          ...
          <View style={styles.choiceSection}>
            <View style={styles.choiceCard}>
              ...
              <Text style={styles.choiceDesc}>
                Follow step-by-step instructions for conducting the presumptive field test, or skip directly to evidence capture.
              </Text>
            </View>

            <TouchableOpacity
              style={[styles.choiceButton, styles.choicePrimary]}
              onPress={() => {
            ...
            <TouchableOpacity
              style={[styles.choiceButton, styles.choiceSecondary]}
        ...
        {step === 'steps' && (
          <View style={styles.stepsSection}>
            <View style={styles.stepProgress}>
              ...
              <View style={styles.progressBar}>
                <View
                  style={[
                    styles.progressFill,
                    { width: `${((guidedStepIndex + 1) / GUIDED_STEPS.length) * 100}%` },
                  ]}
                />
              </View>
            </View>
            ...
            <View style={styles.stepNav}>
              <TouchableOpacity
                style={[styles.stepNavButton, guidedStepIndex === 0 && styles.stepNavDisabled]}
                disabled={guidedStepIndex === 0}
                onPress={() => setGuidedStepIndex((i) => Math.max(0, i - 1))}
              >
              ...
              {guidedStepIndex < GUIDED_STEPS.length - 1 ? (
                <TouchableOpacity
                  style={styles.stepNavNext}
                  onPress={() => setGuidedStepIndex((i) => i + 1)}
                >
        ...
        {step === 'camera' && (
          <View style={styles.cameraSection}>
            ...
            <TouchableOpacity
              style={[styles.primaryButton, !capturedImageUri && styles.primaryDisabled]}
        ...
        {step === 'result' && (
          <View style={styles.resultSection}>
            ...
            <ResultButton
              label="Positive"
              icon={<CheckCircle2 size={24} color={Colors.red[400]} strokeWidth={2} />}
            ...
            <ResultButton
              label="Negative"
              icon={<XCircle size={24} color={Colors.green[400]} strokeWidth={2} />}
              selected={result === 'Negative'}
              onPress={() => setResult('Negative')}
              accentColor={Colors.green[400]}
            />
            <ResultButton
              label="Inconclusive"
              icon={<HelpCircle size={24} color={Colors.amber[400]} strokeWidth={2} />}
              selected={result === 'Inconclusive'}
              onPress={() => setResult('Inconclusive')}
              accentColor={Colors.amber[400]}
            />

            <TouchableOpacity
              style={[styles.saveButton, !result && styles.primaryDisabled]}
              disabled={!result}
              onPress={handleSave}
            >
      ...
      <CameraCapture
        ...
        onCapture={(uri) => setCapturedImageUri(uri)}
      />
    </View>
  );
}
...
function ResultButton({
  ...
  return (
    <TouchableOpacity
      style={[styles.resultButton, selected && { borderColor: accentColor, borderWidth: 2 }]}
      onPress={onPress}
      activeOpacity={0.7}
    >
      {icon}
...
const styles = StyleSheet.create({
  ...
  audioBadge: {
    ...
    paddingVertical: 6,
    backgroundColor: 'rgba(20, 145, 139, 0.12)',
    borderRadius: 8,
